...
def crop(img, center, scale, output_size, rot=0):
    ...
    if sf < 2:
        ...
        if new_size < 2:
            ...
            img = np.array(Image.fromarray(np.uint8(img)).resize((new_ht, new_wd)))
    ...
    if not rot == 0:
        ...
        new_img = np.array(Image.fromarray(np.uint8(new_img)).rotate(rot))
        new_img = new_img[pad:-pad, pad:-pad]
    # new_img = scipy.misc.imresize(new_img, output_size)
    new_img = np.array(Image.fromarray(np.uint8(new_img)).resize(output_size))
    return new_img
